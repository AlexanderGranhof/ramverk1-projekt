...
class User extends Database
{
    ...
    public function score($uid) {
        ...
        $stmt = $this->db->prepare("SELECT (SELECT SUM(score) * 10 FROM comment_votes WHERE user_id = :uid1)  + (SELECT COUNT(id) FROM posts WHERE user_id = :uid2) + (SELECT COUNT(id) FROM comments WHERE user_id = :uid3) + (SELECT SUM(comment_votes.score) * 2 FROM comments LEFT OUTER JOIN comment_votes ON comment_votes.comment_id = comments.id WHERE comments.answer = 1 AND comments.user_id = :uid4) + (SELECT SUM(score) FROM post_votes WHERE user_id = :uid5) AS sum_score;");
